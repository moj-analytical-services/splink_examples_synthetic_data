...
df_source = spark.read.parquet(output_paths["source_nodes_path"])
...
df_e_with_labels = df_e_with_labels.withColumn("bf_temp", f.expr(expr))
df_e_with_labels = df_e_with_labels.withColumn(
    f"{score_colname}", f.expr("power(2, bf_temp)/(1+power(2,bf_temp))")
)
